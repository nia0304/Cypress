...
describe("Data Penguji", () => {
    ...
    it("Hapus data penguji", () => {
        ...
        cy.get("header").scrollIntoView();
        cy.get(".alert").should("contain" , "Penghapusan data Penguji Seleksi berhasil");
    });

      it.only("Duplikat data penguji", () => {
        cy.visit('/spmb/list_jadwal');
        cy.get("#periode").select("2020/2021 Genap");
        cy.wait(5000);
        cy.get("#jalur").select("Mandiri");
        ...
        cy.get("#insert-row-ms > :nth-child(4) > .btn > .fa").click();
        cy.get("header").scrollIntoView();
        cy.get(".alert").should("contain" , "Bentrok Dengan Seleksi TES CBT Tanggal 1 Oktober 2021 Jam 11:00:00 - 14:00:00 ")
    });
});
